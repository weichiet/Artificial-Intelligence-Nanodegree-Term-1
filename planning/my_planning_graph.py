...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Performing state node[level] -> action node[level]
        
        #Set of a_nodes - empty to start
        self.a_levels.append(set())  
        
        #For each action of the problem
        for action in self.all_actions:
        	# Create a new action node
        	action_node = PgNode_a(action)
        	
        	# For each state node of sama level 
        	for state_node in self.s_levels[level]:
        		# Add the action node if PgNode_a has prenodes that are a subset of the previous S level
    ...
    def add_literal_level(self, level):
        ...
        #Set of s_nodes - empty to start
        self.s_levels.append(set())
        
        #For each A node in the previous level
        for action_node in self.a_levels[level-1]:
        	# For each S node that represents the effect of previous A level
        	for state_node in action_node.effnodes:
        		# Add S node to current S level
        		self.s_levels[level].add(state_node)
        		
        		# Connect S node and A node
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level, list_of_states in enumerate(self.s_levels):
                if found:
                    break
